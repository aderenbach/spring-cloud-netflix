...
public class ProxyRouteLocatorTests {
    ...
    public void testIgnoredRouteIncludedIfConfiguredAndDiscovered() {
        ...
        Map<String, String> routesMap = routeLocator.getRoutes();
        String serviceId = routesMap.get(getMapping("foo").getRoute());
    ...
    public void testIgnoredRoutePropertiesRemain() {
        ...
        this.properties.setIgnoredServices(Collections.singletonList("*"));
        given(this.discovery.getServices()).willReturn(Collections.singletonList("foo"));
        LinkedHashMap<ProxyRouteLocator.Path, ZuulRoute> routes = routeLocator.locateRoutes();
    ...
    public void testIgnoredRouteNonServiceIdPathRemains() {
        ...
        given(this.discovery.getServices()).willReturn(Collections.singletonList("foo"));
        LinkedHashMap<ProxyRouteLocator.Path, ZuulRoute> routes = routeLocator.locateRoutes();
        ZuulRoute actual = routes.get(new ProxyRouteLocator.Path("/**"));
        assertNotNull("routes ignored foo", actual);
    ...
    public void testIgnoredRouteIncludedIfConfiguredAndNotDiscovered() {
        ...
        Map<String, String> routesMap = routeLocator.getRoutes();
        String id = routesMap.get(getMapping("foo").getRoute());
    ...
    @Test
    public void testGetMatchingPathWithMethod() throws Exception {
        ProxyRouteLocator routeLocator = new ProxyRouteLocator("/", this.discovery,
                this.properties);
        this.properties.getRoutes().put("without", new ZuulRoute("/foo", "without"));
        this.properties.getRoutes().put("get", new ZuulRoute("/foo", "get","GET"));
        this.properties.getRoutes().put("push", new ZuulRoute("/foo", "push", "PUSH"));
        this.properties.setStripPrefix(false);

        routeLocator.getRoutes(); // force refresh
        ProxyRouteSpec route = routeLocator.getMatchingRoute("/foo","GET");
        assertEquals("get", route.getLocation());
        ProxyRouteSpec route2 = routeLocator.getMatchingRoute("/foo","PUSH");
        assertEquals("push", route2.getLocation());
        ProxyRouteSpec route3 = routeLocator.getMatchingRoute("/foo");
        assertEquals("without", route3.getLocation());
        ProxyRouteSpec route4 = routeLocator.getMatchingRoute("/foo","DELETE");
        assertEquals("without", route4.getLocation());
    }
    ...
    protected void assertMapping(Map<String, String> routesMap, String expectedRoute,
                                 String key) {
        String mapping = getMapping(key).getRoute();
        String route = routesMap.get(mapping);
        assertEquals("routesMap had wrong value for " + mapping, expectedRoute, route);
    }

    private ProxyRouteLocator.Path getMapping(String serviceId) {
        return new ProxyRouteLocator.Path("/" + serviceId + "/**");
    }
